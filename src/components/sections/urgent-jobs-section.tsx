...
export function UrgentJobsSection() {
  const { jobs, loading, error, retry } = usePublicJobs({ urgent: true, limit: 6 });
  const { mode } = useLanguageMode();

  // Filter client-side to ensure only actively urgent jobs display
  const activeUrgentJobs = jobs.filter(isUrgentActive);
    ...
    <div id="urgent-jobs">
      ...
      <ScrollReveal className="mb-12">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          {mode !== 'en' && (
            <p className="font-mono text-sm font-medium tracking-[0.2em] text-amber-400/80 uppercase">
              Urgent Hiring
            </p>
          )}
        </div>
        ...
        <p className="mt-2 text-base leading-relaxed text-muted-foreground">
          {mode === 'en'
            ? 'Positions that need filling right now'
            : '当前急需填补的岗位机会'}
        </p>
      </ScrollReveal>
        ...
        <ScrollReveal>
          <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
            <p className="mb-2 text-sm font-medium text-foreground">
              {mode === 'en' ? 'Failed to load urgent positions' : '急招岗位加载失败'}
            </p>
      ...
      {/* Empty state — first-class scenario */}
      {!loading && !error && activeUrgentJobs.length === 0 && (
        <ScrollReveal>
          <div className="rounded-2xl border border-border bg-card/50 p-10 text-center">
            ...
            <p className="mb-2 text-sm font-medium text-foreground">
              {mode === 'en' ? 'No urgent positions right now' : '当前暂无急招岗位'}
            </p>
      ...
      {!loading && !error && activeUrgentJobs.length > 0 && (
        <div className="border-t border-white/[0.06]">
          {activeUrgentJobs.map((job, index) => (
            <UrgentJobRow key={job.id} job={job} index={index} />
          ))}
        </div>
      )}
      ...
      {!loading && !error && activeUrgentJobs.length > 0 && (
        <div className="mt-6 text-center">
          <Link
            href="/opportunities?urgent=true"
            className="inline-flex items-center gap-2 rounded-full bg-surface-glass px-5 py-2.5 text-sm font-medium text-foreground backdrop-blur-sm transition-all duration-300 hover:bg-surface-glass-hover focus:outline-none focus:ring-2 focus:ring-accent/30"
          >
            {mode === 'en' ? 'View All Urgent Jobs' : '查看全部急招岗位'}
            <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      )}
...
function UrgentJobRow({ job, index }: { job: PublicJob; index: number }) {
  ...
  return (
    <ScrollReveal key={job.id} delay={index * 0.05}>
      <Link
        href={`/jobs/${job.slug}`}
        onClick={() => trackEvent('job_view', { job_id: job.id, source: 'urgent' })}
